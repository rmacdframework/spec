...
- [Governance Packs](docs/governance-packs/) — *New in SDK 0.11.0 (`rmacd.packs`):* declarative, reusable, signable packs that map a tool surface to RMACD terms (operation/tier/target) so agents are governed off the shelf — `load_packs(["aws", "kubectl", "jira"])` — plus an AI-compile authoring workflow and 19 built-in packs ([overview](docs/governance-packs/README.md), [design](docs/governance-packs/design.md), [roadmap](docs/governance-packs/roadmap.md), [authoring guide](docs/governance-packs/authoring-guide.md))
