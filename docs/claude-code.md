...
| Session-internal tools (`Task`, `Agent`, `TodoWrite`, `AskUserQuestion`, `Skill`, `SlashCommand`, `ExitPlanMode`, `BashOutput`, `KillShell`, ...) | Read on `public` (they orchestrate the session, not external data) — still governed, so a profile that denies Read denies them too |
