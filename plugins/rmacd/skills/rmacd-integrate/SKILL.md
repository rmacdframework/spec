...
- **Install name ≠ import name.** `pip install "rmacd-framework>=0.13"` but
...
≥ 0.13, the profile validates, and env vars are bound. Then smoke-test with
...
## Claude Code session governance (requires rmacd-framework>=0.13)
...
`/rmacd:status` command reports the bound profile. Both require `rmacd-framework>=0.13`;
