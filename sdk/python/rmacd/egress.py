...
class PolicyDrivenEgressGate:
    ...
    @staticmethod
    def _matches_allowlist(destination: str, allowed: list[str]) -> bool:
        # Match by substring or exact tag. The allow-list entries in the
        # profile schema are free-form labels (e.g. "tenant-hosted-llm")
        # rather than URLs, so we check both: direct equality with the
        # destination *and* substring containment for URL destinations
        # tagged with the label.
        for entry in allowed:
            if entry == destination:
                return True
            # Permit URL-ish matches: if entry is a hostname/label that
            # appears in the destination string, accept. This is
            # deliberately loose; tighter matchers should be implemented
            # by integrators with concrete destination schemas.
            if entry in destination:
                return True
        return False

    @staticmethod
    def _extract_host(destination: str) -> str | None:
        # Try to parse the destination as a URL and return the host. Fall
        # back to None for labels that aren't URLs (e.g. "internal-kb"),
        # which are handled by the allow-list rule instead.
        try:
